...
export default async (req: Request, res: Response) => {
    ...
    const nameservers: any = await db.all(
        'SELECT nameserver FROM nameservers WHERE entry=?',
        req.params.id
    );
    const addresses: any = await db.all(
        'SELECT address FROM addresses WHERE entry=?',
        req.params.id
    );
    if (!entry) {
        ...
        entry.lookups = {};
        entry.nameservers = nameservers.map(e => e.nameserver);
        entry.addresses = addresses.map(e => e.address);
